...
%%% Average corr per word after n iter for the 2 distances
iter_RES_corr_combo_F0_perword = zeros(size(word_list,1),1);
iter_RES_corr_combo_int_perword = zeros(size(word_list,1),1);
iter_RES_corr_combo_F0_int_perword = zeros(size(word_list,1),1);

%%% Average corr per feature after n iter for the 2 distances
iter_final_corr_combo_F0 = 0;
iter_final_corr_combo_int = 0;
iter_final_corr_combo_F0_int = 0;

for it = 1:n_iter

    %%% Final results (for 1 iter)
    RES_corr_combo_F0_perword = zeros(length(word_list),1);
    RES_corr_combo_int_perword = zeros(length(word_list),1);
    RES_corr_combo_F0_int_perword = zeros(length(word_list),1);
...
%         clusters_F0 = kMeans(soft_F0_nat,k_F0,max_kmeans);
        ...
        %%%               COMBINE DISTANCES
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        combo_F0 = mean([wdist_F0,durdist_F0],2);
        combo_int = mean([wdist_int,durdist_int],2);
        combo_F0_int = mean([wdist_F0_int,durdist_F0_int],2);
       
        %%% Correlation between the combo auto scores and the real ones
        %%% for one word
        corr_combo_F0_word = corr(-combo_F0,label);
        corr_combo_int_word = corr(-combo_int,label);
        corr_combo_F0_int_word = corr(-combo_F0_int,label); 

        %%% Input the previous correlation scores to RES_corr
        RES_corr_combo_F0_perword(i) = corr_combo_F0_word;
        RES_corr_combo_int_perword(i) = corr_combo_int_word;
        RES_corr_combo_F0_int_perword(i) = corr_combo_F0_int_word;
        
        %%% When all speakers have been evaluated
    end
    
    iter_RES_corr_combo_F0_perword = ...
        iter_RES_corr_combo_F0_perword + ...
        atanh(RES_corr_combo_F0_perword);
    iter_RES_corr_combo_int_perword = ...
        iter_RES_corr_combo_int_perword + ...
        atanh(RES_corr_combo_int_perword);
    iter_RES_corr_combo_F0_int_perword = ...
        iter_RES_corr_combo_F0_int_perword + ...
        atanh(RES_corr_combo_F0_int_perword);
    ...
    for i = 1:length(word_list)
        final_corr_durdist_F0 = final_corr_durdist_F0 + atanh(RES_corr_combo_F0_perword(i));
        final_corr_durdist_int = final_corr_durdist_int + atanh(RES_corr_combo_int_perword(i));
        final_corr_durdist_F0_int = final_corr_durdist_F0_int + atanh(RES_corr_combo_int_perword(i));
    end
    ...
    iter_final_corr_combo_F0 = iter_final_corr_combo_F0 + atanh(final_corr_durdist_F0);
    iter_final_corr_combo_int = iter_final_corr_combo_int + atanh(final_corr_durdist_int);
    iter_final_corr_combo_F0_int = iter_final_corr_combo_F0_int + atanh(final_corr_durdist_F0_int);
end

iter_RES_corr_combo_F0_perword = tanh(iter_RES_corr_combo_F0_perword/n_iter);
iter_RES_corr_combo_int_perword = tanh(iter_RES_corr_combo_int_perword/n_iter);
iter_RES_corr_combo_F0_int_perword = tanh(iter_RES_corr_combo_F0_int_perword/n_iter);

iter_final_corr_combo_F0 = tanh(iter_final_corr_combo_F0/n_iter);
iter_final_corr_combo_int = tanh(iter_final_corr_combo_int/n_iter);
iter_final_corr_combo_F0_int = tanh(iter_final_corr_combo_F0_int/n_iter);
